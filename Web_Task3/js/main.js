...
nm.textContent=list[0].name;
ret.textContent=list[0].ratingRevievs;


mod.append(nm);
mod.append(ret);

if(list[0].price.newUan==undefined)
{
    let price = document.createElement('div');
    price.className='price';
    price.textContent=list[0].price;
    mod.append(price);
}
else
{
    let newpr = document.createElement('div');
    let oldpr = document.createElement('strike');
    oldpr.className='strike';
    newpr.className='nprice';
    oldpr.textContent=list[0].price.oldUan;
    newpr.textContent=list[0].price.newUan;
    mod.append(oldpr);
    mod.append(newpr);
}
...
{
    let mod2 = mod.cloneNode(true);
    mod2.firstChild.textContent=list[index].name;
    mod2.childNodes[1].textContent=list[index].ratingRevievs;
    div.append(mod2);
    mod2.removeChild(mod2.childNodes[3]);

    
    mod2.removeChild(mod2.childNodes[2]);
    if(list[index].price.newUan==undefined)
{
    let price = document.createElement('div');
    price.className='price';
...
else
{
    ...
    oldpr.className='strike';
    newpr.className='nprice';
    oldpr.textContent=list[index].price.oldUan;
    newpr.textContent=list[index].price.newUan;
    mod2.append(oldpr);
    mod2.append(newpr);
}
}
